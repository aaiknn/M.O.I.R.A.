...
async def dbSelftest(self, situation):
  try:
    ...
    situation.errors.appendMessage(ugh.database_connection_aborted_error)
  except Exception as e:
    self.tism.setSystemState('DB', 'DOWN')
    situation.exceptions.appendMessage(f'{ugh.database_connection_error}: {e}')
  else:
    if not selftest:
      self.tism.setSystemState('DB', 'DOWN')
      situation.errors.appendMessage(ugh.database_connection_error_unknown)
...
async def eonetSelftest(self, situation):
  ...
  try:
    ...
    situation.exceptions.appendMessage(f'{ugh.eonet_selftest_failed}: {e}')
  ...
  if len(categories) == 0:
    situation.warnings.appendMessage(warn.eonet_categories_not_set)
...
async def openAiSelftest(self, situation):
  ...
  try:
    ...
    situation.errors.appendMessage(exceptionMessage)
    return False

  else:
    self.tism.setSystemState('AI', 'UP')
    situation.status.appendMessage(f'{syx.openai_selftest_response.format(engine)}: `{res.text[0]}`')
    return response
